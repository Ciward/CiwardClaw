...
export async function steerActiveSessionWithOptionalDeliveryWait(
  ...
  const images = modelHasVision ? options?.images : undefined;
  if (options?.waitForTranscriptCommit !== true) {
    if (options?.userTurnTranscriptRecorder) {
      await activeSession.steer(text, images, options.userTurnTranscriptRecorder);
    } else if (images) {
      await activeSession.steer(text, images);
    } else {
