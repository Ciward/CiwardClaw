...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  it("admits provider ground truth when owning context engine assembly fails", async () => {
    let sawPrompt = false;
    const hugeHistory = "large raw history ".repeat(2_000);
  ...
  it("diagnoses preassembly pressure without blocking the provider", async () => {
    let sawPrompt = false;
    const hugeHistory = "large raw history ".repeat(2_000);
  ...
  it("keeps owning context-engine preassembly pressure diagnostic-only", async () => {
    let sawPrompt = false;
    const hugeHistory = "large raw history ".repeat(2_000);
...
describe("runEmbeddedAttempt tool-result guard budget wiring", () => {
  ...
  it("admits aggregate prompt-history pressure after request-local truncation", async () => {
    let sawPrompt = false;
    const sessionMessages: AgentMessage[] = [{ role: "user", content: "seed", timestamp: 1 }];
  ...
  it("admits protected trailing aggregate pressure to the provider", async () => {
    let sawPrompt = false;
