...
// Mirrors the in-memory grace window in `src/gateway/server-chat.ts`.
...
function clearPendingErrorTimer(runId: string): void {
  const timer = pendingErrorTimers.get(runId);
  if (!timer) {
    return;
  }
  clearTimeout(timer);
...
async function readStore(storePath: string): Promise<InflightAgentRunsStore> {
  ...
  const record = parsed as Partial<InflightAgentRunsStore>;
  if (record.version !== STORE_VERSION || !record.runs || typeof record.runs !== "object") {
    return { version: STORE_VERSION, runs: {} };
  }
  return { version: STORE_VERSION, runs: record.runs };
}
...
async function removeRunFromStore(storePath: string, runId: string): Promise<void> {
  ...
  await storeLock(async () => {
    ...
    if (!store.runs[cleanedRunId]) {
      return;
    }
    delete store.runs[cleanedRunId];
...
export async function removeInflightAgentRun(
  ...
  clearPendingErrorTimer(cleanedRunId);
  await removeRunFromStore(resolveStorePath(env), cleanedRunId);
}

/**
 * Batch-update resumeCount/lastResumeAt for all given runIds in one
...
export async function markInflightAgentRunsResumed(
  ...
  const cleanedRunIds = runIds.map((id) => id.trim()).filter(Boolean);
  ...
  await storeLock(async () => {
    const store = await readStore(storePath);
    let changed = false;
    for (const runId of cleanedRunIds) {
 ...
 *
 * - On lifecycle end: remove immediately.
