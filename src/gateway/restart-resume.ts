...
import { readRestartSentinel, type RestartSentinel } from "../infra/restart-sentinel.js";
...
type ResumeTypingChannelRuntime = ReturnType<typeof createPluginRuntime>["channel"] & {
  telegram?: {
    typing?: {
      start?: (params: {
        to: string;
...
function attachResumeTypingLifecycleBridge(params: {
  ...
  const startLease = async () => {
    ...
    leaseStartPromise = (async () => {
      try {
        const runtime = params.resolveChannelRuntime() as ResumeTypingChannelRuntime;
        const nextLease =
...
export async function maybeResumeInflightAgentRunsAfterRestart(params: {
  ...
  const inflightRuns = await listInflightAgentRuns(env);
  const runAgent = params.runAgent ?? agentCommandFromIngress;
  ...
  for (const entry of inflightRuns) {
    ...
    void runAgent(resumeOpts, params.runtime, params.deps)
      .catch((error) => {
        logVerbose(`restart recovery: resumed run ${runId} failed: ${String(error)}`);
      })
      .finally(() => {
        cleanupTyping();
      });
  ...
  await markInflightAgentRunsResumed(resumedRunIds, env).catch(() => {});

  return { resumed: resumedRunIds.length, considered: inflightRuns.length, skipped: false };
}
